...
public class StringToTimeTest {
  ...
  public void testISO8601() {
    ...
    assertEquals(new Date(cal.getTimeInMillis()),
        new StringToTime("1981-10-26T15:25:02.435+00:00", now));
    assertEquals(new Date(cal.getTimeInMillis()),
        new StringToTime("1981-10-26T15:25:02.435Z", now));
    assertEquals(new Date(cal.getTimeInMillis()),
        new StringToTime("1981-10-26T15:25:02.435", now));

    cal.set(Calendar.MILLISECOND, 0);
    cal.set(Calendar.ZONE_OFFSET, 0);

    assertEquals(new Date(cal.getTimeInMillis()),
        new StringToTime("1981-10-26T15:25:02", now));
  ...
  public void testVariousExpressionsOfTimeOfDay() {
    ...
    cal.set(Calendar.MILLISECOND, 0);
    assertEquals(new Date(cal.getTimeInMillis()),
        new StringToTime("October 26, 1981 3:27:00 PM", now));
    ...
    assertEquals(new StringToTime("yesterday evening", now),
        new StringToTime("yesterday 5PM", now));
  }
  ...
  public void testTomorrowMorning() {
    Date now = now();
    assertEquals(new StringToTime("this morning +24 hours", now),
        new StringToTime("tomorrow morning", now));
  }
  ...
  public void testTomorrowAfternoon() {
    Date now = now();
    assertEquals(new StringToTime("this afternoon +24 hours", now),
        new StringToTime("tomorrow afternoon", now));
  }

  @Test
  public void testTomorrowEvening() {
    Date now = now();
    assertEquals(new StringToTime("this evening +24 hours", now),
        new StringToTime("tomorrow evening", now));
  }

  @Test
  public void testTomorrowNight() {
    Date now = now();
    assertEquals(new StringToTime("tonight +24 hours", now),
        new StringToTime("tomorrow night", now));
  }
  ...
  public void testLongHand() throws Exception {
    assertEquals(new SimpleDateFormat("M/d/y").parse("10/26/1981"),
        new StringToTime("October 26, 1981"));
    assertEquals(new SimpleDateFormat("M/d/y").parse("10/26/1981"),
        new StringToTime("Oct 26, 1981"));

    assertEquals(new SimpleDateFormat("M/d/y").parse("10/26/1981"),
        new StringToTime("26 October 1981"));
    assertEquals(new SimpleDateFormat("M/d/y").parse("10/26/1981"),
        new StringToTime("October 26, 1981"));
    assertEquals(new SimpleDateFormat("M/d/y").parse("10/26/1981"),
        new StringToTime("October 26 1981"));
  ...
  public void testWithSlashesMonthAndDate() throws Exception {
    ...
    cal.set(Calendar.MONTH, Calendar.OCTOBER);
    cal.set(Calendar.DATE, 26);
    assertEquals(new Date(cal.getTimeInMillis()), new StringToTime("10/26", now));
  }
  ...
  public void testMonth() throws Exception {
    ...
    assertEquals(cal.get(Calendar.YEAR),
        new StringToTime("January", now).getCal().get(Calendar.YEAR));
    assertEquals(cal.get(Calendar.YEAR),
        new StringToTime("December", now).getCal().get(Calendar.YEAR));
  }
  ...
  public void testDayOfWeek() throws Exception {
    ...
    // if today's day of the week is greater than or equal to our test day of the week (Wednesday)
      if (cal.get(Calendar.DAY_OF_WEEK)
          >= 3) // then the day of the week on the date returned should be next week
      {
  ...
  public void testRelativeDate() throws Exception {
    ...
    final Object date1 = StringToTime.date("22 February 1735 +11 day");
