...
import info.bliki.extensions.scribunto.engine.lua.interfaces.MwHash;
...
public class ScribuntoLuaEngine extends ScribuntoEngineBase implements MwInterface {
  ...
  protected LuaValue loadFunction(String functionName, Prototype prototype, Frame frame)
  ...
  private void stubWikiBase() {
    ...
    wikibase.set("getBestStatements", new TwoArgFunction() {
      @Override
      public LuaValue call(LuaValue id, LuaValue property) {
        return new LuaTable();
      }
    });
    ...
    try (InputStream is = globals.finder.findResource(filename)) {
      if (is == null) {
        throw new FileNotFoundException(
            "could not find '" + filename + "'. Make sure it is on the classpath.");
      }
  ...
  public LuaTable getInterface() {
    ...
    table.set("setTTL", defaultFunction());
    table.set("addWarning", defaultFunction());
    table.set("loadJsonData", loadJsonData());
    return table;
  }
  ...
  private LuaValue loadJsonData() {
    return new OneArgFunction() {
      @Override
      public LuaValue call(LuaValue title) {
        throw new LuaError("loadJsonData not yet implemented by ScribuntoLuaEngine");
      }
    };
  }
  ...
  private Prototype loadAndCache(InputStream code, ParsedPageName chunkName)
      throws ScribuntoException {
    try {
  ...
  /**
   * Use Java's UTF-8 decoding instead of luaj's broken implementation, since it does not handle
   * surrogates properly.
   */
